    ...
    firebase.database().ref('message_reports').orderByChild('timestamp').on('value', function (snap) {
        ...
        el.innerHTML = reports.map(function (entry) {
            ...
            return '<div style="background:rgba(239,68,68,0.05); border:1px solid rgba(239,68,68,0.15); border-radius:12px; padding:14px; margin-bottom:8px;">' +
                ...
                '<button onclick="if(confirm(\'Banir o autor desta mensagem?\')) { banUserKord(\'' + r.authorId + '\', \'Violação das Diretrizes\', false); adminDeleteReport(\'' + id + '\'); showAdminToast(\'Autor banido!\', \'#f59e0b\'); }" style="background:rgba(245,158,11,0.2); color:#f59e0b; border:1px solid rgba(245,158,11,0.3); padding:4px 12px; border-radius:8px; cursor:pointer; font-size:12px;">⛔ Banir Autor (10D)</button>' +
                '<button onclick="if(confirm(\'Isso vai banir quem FEZ a denúncia (falso report). Confirmar?\')) { banUserKord(\'' + r.reporterId + '\', \'Denuncia Falsa / Abuso\', false); adminDeleteReport(\'' + id + '\'); showAdminToast(\'Reporter banido!\', \'#c084fc\'); }" style="background:rgba(139,92,246,0.2); color:#c084fc; border:1px solid rgba(139,92,246,0.3); padding:4px 12px; border-radius:8px; cursor:pointer; font-size:12px;">🚫 Banir Reporter (Falso Report)</button>' +
                '</div>' +
                '</div>';
